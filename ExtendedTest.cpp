...
void testQuantity() {
	...
	Matrix m(200, 300);
	for (int i = (int)m.nrLines() / 2; i < m.nrLines(); i++) {
		for (int j = 0; j <= m.nrColumns() / 2; j++)
		{
			int v1 = j;
			int v2 = (int)m.nrColumns() - v1 - 1;
			if (i % 2 == 0 && v1 % 2 == 0)
				m.modify(i, v1, i*v1);
			else
				if (v1 % 3 == 0)
...
void testSetMainDiagonal(){
    std::cout<<"Test extra functionality"<<std::endl;
    Matrix nonsquareMatrix{10, 20};
    try{
        nonsquareMatrix.setMainDiagonal(1);
        assert(false);
    }catch(const std::exception& err){
        assert(true);
    }
    Matrix squareMatrix{1000, 1000};
    squareMatrix.setMainDiagonal(13);
    for(int i = 0; i < 1000; i ++)
        assert(squareMatrix.element(i, i) == 13);

    squareMatrix.setMainDiagonal(NULL_TELEM);
    for(int i = 0 ; i < 1000; i ++){
        assert(squareMatrix.element(i, i) == NULL_TELEM);
    }

    squareMatrix.setMainDiagonal(13);
    for(int i = 0 ; i < 1000; i ++){
        for(int j = 0; j < 1000; j ++){
            if(squareMatrix.element(i,j) != NULL_TELEM)
                assert(i == j);
        }
    }
}
